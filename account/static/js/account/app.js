...
var is_url_signin = url_atual.indexOf('signin')!=-1?true:false; 
var is_url_signup = url_atual.indexOf('signup')!=-1?true:false; 

var is_url_login = url_atual.indexOf('login')!=-1?true:false; 
var is_url_cadastrar = url_atual.indexOf('cadastrar')!=-1?true:false; 

window.onload = () => { 
	// começar no topo
	  window.scrollTo(0, 0);
	  // selecionar a tela que deve começar
	if(is_url_login || is_url_signin){
		ativar_sign_in_js()
	}else if(is_url_cadastrar || is_url_signup){
		ativar_sign_up_js()
	}
